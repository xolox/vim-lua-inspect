...
function! xolox#luainspect#misc#option#get(name, ...) " {{{1
...
function! xolox#luainspect#misc#option#split(value) " {{{1
  " Given a multi-value Vim option like ['runtimepath'] [rtp] this returns a
  " list of strings. For example:
  "
  "     :echo xolox#luainspect#misc#option#split(&runtimepath)
...
function! xolox#luainspect#misc#option#join(values) " {{{1
  " Given a list of strings like the ones returned by
  " `xolox#luainspect#misc#option#split()`, this joins the strings together into a
...
function! xolox#luainspect#misc#option#split_tags(value) " {{{1
  " Customized version of `xolox#luainspect#misc#option#split()` with specialized
...
function! xolox#luainspect#misc#option#join_tags(values) " {{{1
  " Customized version of `xolox#luainspect#misc#option#join()` with specialized
  " handling for Vim's ['tags' option] [tags].
  let values = copy(a:values)
...
function! xolox#luainspect#misc#option#eval_tags(value, ...) " {{{1
  ...
  for pattern in xolox#luainspect#misc#option#split_tags(a:value)
    " Make buffer relative pathnames absolute.
    if pattern =~ '^\./'
      let directory = xolox#luainspect#misc#escape#substitute(expand('%:p:h'))
      let pattern = substitute(pattern, '^.\ze/', directory, '')
    endif
    " Make working directory relative pathnames absolute.
    if xolox#luainspect#misc#path#is_relative(pattern)
      let pattern = xolox#luainspect#misc#path#merge(getcwd(), pattern)
    endif
